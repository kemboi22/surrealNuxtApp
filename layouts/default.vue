...
import {
  ...
  HomeIcon,
  UsersIcon,
  XMarkIcon, CircleStackIcon, InformationCircleIcon
...
const navigation = [
  ...
  { name: 'Connection Information', href: '/info', icon: InformationCircleIcon },
]
const userNavigation = [
  { name: 'Your profile', href: '#' },
  { name: 'Sign out', href: '#' },
]
const currentRoute = (path: string) => useRoute().fullPath == path
...
<template>
  <div>
    <HeadlessTransitionRoot as="template" :show="sidebarOpen">
      <HeadlessDialog as="div" class="relative z-50 lg:hidden" @close="sidebarOpen = false">
        <HeadlessTransitionChild as="template" enter="transition-opacity ease-linear duration-300"
                                enter-from="opacity-0" enter-to="opacity-100" leave="transition-opacity ease-linear duration-300" leave-from="opacity-100" leave-to="opacity-0">
              ...
              <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-gray-600 px-6 pb-4">
                ...
                <nav class="flex flex-1 flex-col">
                  <ul role="list" class="flex flex-1 flex-col gap-y-7">
                    ...
                    <li class="mt-auto">
                      <a href="#"
                         class="group -mx-2 flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6 text-gray-200 hover:bg-gray-700 hover:text-white">
                        <Cog6ToothIcon class="h-6 w-6 shrink-0 text-gray-200 group-hover:text-white"
                                       aria-hidden="true" />
                        Settings
                      </a>
                    </li>
    ...
    <div class="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col">
      <!-- Sidebar component, swap this element with another sidebar if you like -->
      <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-black/90 px-6 pb-4">
        <div class="flex h-16 shrink-0 items-center">
          <img class="h-8 w-auto" src="https://tailwindui.com/img/logos/mark.svg?color=white" alt="Your Company" />
        </div>
        <nav class="flex flex-1 flex-col">
